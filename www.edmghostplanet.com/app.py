...
def get_pages_data(url, range_form=1, range_to=1):
	...
	for iter_page in range(range_form, range_to+1 ):
		...
		all_trecks = soup.find('ul',{'class': 'products'}).find_all("li", {'class': 'product'})
		for card_track in all_trecks: 
			# track_info = card_track.find('div', {"class": 'track-info'} )
			card_data = {
				"last_update" : strftime("%Y %H:%M:%S", gmtime()),
				"name" : valid_file_name( card_track.find('h2',{'class':'woocommerce-loop-product__title'}).string.strip() ) ,
			}
			try: 
				card_data['url_to'] = card_track.find('div',{'class': 'is-single-player'}).attrs['data-source']
			except: 
				card_data['url_to'] = ''
			...
			# try: 
			# 	card_data['sale'] = [ i if i != None else '' for i in card_track.select('audio') ][0]
			# 	card_data['sale'] = True
			# except IndexError: 
			# 	card_data['sale'] = False
...
all_data_from_site = get_pages_data(url='https://www.edmghostplanet.com/track-store/?product-page=', range_form=range_from_, range_to=range_to_)
